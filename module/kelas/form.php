<?php
    ...
    if ($_GET['form_status'] == 'kelas_edit') {
        $id=$_GET['id'];
        $result=$db->get_row("SELECT * FROM kelas WHERE id='$id'");
    }

    $id = (!empty($result->id)) ? $result->id : '';
    $kelas = (!empty($result->kelas)) ? $result->kelas : '';

    $form_title = ($_GET['form_status'] == 'kelas_edit') ? 'Edit Data' : 'Tambah Data';
    ...
    <div class="col-md-6">
        <form method="POST" id="form" data-form-status='<?= $_GET['form_status'] ?>'>
            ...
            <div class="form-group">
                ...
                <input type="text" name="kelas" id="kelas" class="form-control" required value="<?= $kelas; ?>" autocomplete="off"/>
            </div>
